...
function log(t){events.push(new Date().toLocaleTimeString()+" · "+t);$("#log").innerHTML=events.slice(-16).reverse().join("<br>")}
...
function attach(conn){
  ...
  if(!["levi","ikaleth"].includes(role)){conn.on("open",()=>conn.close());return}
  log(role+" solicita conexión");
  conns[role]=conn;
  render();
  const pc=conn.peerConnection;
  if(pc){
    pc.addEventListener("iceconnectionstatechange",()=>log(role+" ICE: "+pc.iceConnectionState));
    pc.addEventListener("connectionstatechange",()=>log(role+" P2P: "+pc.connectionState));
  }
  const timeout=setTimeout(()=>{
    if(!conn.open)log(role+" no abre tras 12 s · ICE: "+(pc?.iceConnectionState||"sin dato"));
  },12000);
  conn.on("open",()=>{clearTimeout(timeout);log(role+" conectado");send(role);render()});
  conn.on("data",m=>{if(m?.type==="move"&&m.role===role&&["N","S","E","W"].includes(m.dir))move(role,m.dir)});
  conn.on("error",e=>log(role+" error: "+errText(e)));
  conn.on("close",()=>{clearTimeout(timeout);if(conns[role]===conn)conns[role]=null;log(role+" desconectado");render()});
}
function makeClientUrl(filename,id){const u=new URL(filename,location.href);u.searchParams.set("host",id);u.searchParams.set("v",Date.now());return u.href}
...
peer.on("error",e=>{$("#peerStatus").textContent="Error de red: "+errText(e);$("#peerStatus").className="status bad";log("PeerJS "+errText(e))});
